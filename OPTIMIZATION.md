...
- **Owned decoding:** benchmark the existing `hex_decode_array` and
  `hex_decode_vec` APIs directly. Explore validating and decoding large inputs
  in one pass, starting with existing kernels. Private output can be discarded
...
- **Calling contexts:** compare ordinary release builds with production LTO,
...
- **Development feedback:** add direct owned-decode measurements and a few
  short/boundary/error cases to the existing focused benchmarks. Keep baseline
  comparison quick; extend native validation to 32-bit x86 when hardware permits.
...
prefer reusing safe code and existing SIMD kernels over adding new unsafe paths.
